...
// We want to, while the player is moving, identify the tiles (based on their coordinates) that are currently visible to the player
...
public class WorldScroll : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Debug.Log('Position:' + playerTilePosition.x + playerTilePosition.y);

        // subtract offset to account for negative positions
    ...
    private int CalculatePositionOnAxis(float currentValue, bool forHorizontal) {
        // If player is on tile 0, they are okay to travel to tiles 1 and 2. But if they are on tile 2, then loop and place tiles 0 and 1 on their right
        // Account for if the position is negative
        // onTileGridPlayerPosition.x or y can be [0, 1, 2], in this case bc world dimensions are 3 x 3;
